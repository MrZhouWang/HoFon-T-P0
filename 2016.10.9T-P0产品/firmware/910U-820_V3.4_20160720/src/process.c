...
//					}
					       ...
					       ||(uart.rBuffer[5] == 0X01 && uart.rBuffer[6] == uart.rIndex-9 && uart.rBuffer[7] ==DISPMODE_TASK && (uart.rBuffer[6]-uart.rBuffer[11]>=8)) )   // 有批次号命令
					{
						...
						//否则，处理
						//任务驱动型信息
//						if(LastSoftGetOkCount == 0 && CurDispMode == DISPMODE_TASK && (task_unfinish_num != 0 || task_finished_num != 0))
//						if(OsCmdSet.LastSoftGetOkCount == 0 && OsCmdSet.CurDispMode == DISPMODE_TASK && (UnfinishTask.num != 0 || FinishTask.num != 0))
//						{
...
//						}
						...
						//按钮驱动型信息
//						if(LastSoftGetOkCount != 0 &&((task_unupdate_num == uart.rBuffer[10])&&(CurDispMode == DISPMODE_TASK)))
//						if(OsCmdSet.LastSoftGetOkCount != 0 &&((UnfinishTask.remain == uart.rBuffer[10])&&(OsCmdSet.CurDispMode == DISPMODE_TASK)))
//						if( ((OsCmdSet.LastSoftGetOkCount == 0)&&(OsCmdSet.CurDispMode == DISPMODE_TASK)&&(UnfinishTask.repeat == TRUE))
//						  ||((OsCmdSet.LastSoftGetOkCount != 0)&&(OsCmdSet.CurDispMode == DISPMODE_TASK)&&(UnfinishTask.remain == uart.rBuffer[10])) )
...
//					else if(uart.rBuffer[5] == 0X01 && uart.rBuffer[6] == uart.rIndex-9 && uart.rBuffer[7] ==DISPMODE_TASK && (uart.rBuffer[6]-uart.rBuffer[11]>=8))
//					{
//						//下发本次显示数据前，上位机接收到的本标签采集到的数据组数，通过此数据来确定发此数据之前已经被上位机采集点的OK键的次数，由此控制task_finished_num
...
//						//否则，处理
//						//任务驱动型信息
//						if(LastSoftGetOkCount == 0 && CurDispMode == DISPMODE_TASK && (UnfinishTask.num != 0 || FinishTask.num != 0))
//						{
//							ACK_Multi_Write_RepeatError();		//回重复收到相同命令的响应
//							break;                                                                
//						}
